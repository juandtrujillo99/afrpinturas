...
class RepositorioUsuario{
	...
	//esta funcion actualiza la biografia del usuario
	public static function actualizar_restricciones($conexion, $id_usuario, $nuevo_restricciones){
		$actualizacion_correcta = false;

		if (isset($conexion)) {
			try{
				$sql = "UPDATE usuarios SET restricciones = :restricciones WHERE id = :id";
				$sentencia = $conexion -> prepare($sql);
				$sentencia -> bindParam(':restricciones', $nuevo_restricciones, PDO::PARAM_STR);
				$sentencia -> bindParam(':id', $id_usuario, PDO::PARAM_STR);
				$sentencia -> execute();
