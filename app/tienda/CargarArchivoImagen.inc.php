...
if (empty($_FILES["archivoImagen"])) {
	...
	if(move_uploaded_file($fileTmpLoc, $carpeta."/".$nombreEmpresa.date("d-m-Y").$string_aleatorio.utf8_decode($fileName))){		
		...
		<input type="text" style="font-size: .5em;"
		value="<?php echo htmlentities("<img src='".RUTA_TIENDA_CATEGORIAS.$nombreEmpresa.date("d-m-Y").$string_aleatorio.$fileName."' class='imagen'>");
			} else {
